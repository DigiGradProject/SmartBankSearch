...
def format_structured_sections(answer: str) -> dict[str, str]:
    ...
    for part in parts:
        ...
        if header in SECTION_HEADERS or any(
            header.startswith(h) for h in SECTION_HEADERS
        ):
...
def wrap_plain_answer(
    ...
    *,
    ...
    source_block = (
        "\n".join(citations_as_source_lines(citations))
        or "- NBE website"
    )
    ...
    if productish:
        ...
        summary_text = cleaned[:600].strip()

        # Avoid duplicating the Summary verbatim as "Key Information" —
        # this happens when the LLM returns one continuous paragraph
        # instead of pre-broken bullet lines.
        key_info_stripped = (
            key_info.replace("-", "")
            .replace("\n", "")
            .strip()
        ...
        if (
            not key_info
            or key_info_stripped[:200] == summary_stripped[:200]
        ):
            key_info_section = ""
        else:
            key_info_section = (
